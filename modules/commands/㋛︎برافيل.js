const fs = require("fs-extra");
const axios = require("axios");

module.exports.config = {
  name: "بروفايل",
  version: "2.1.0",
  hasPermssion: 0,
  credits: "Ayman",
  description: "عرض صورة البروفايل بجودة عالية للإمبراطور ورعيته",
  commandCategory: "صور",
  usages: "[منشن / بالرد / بالأيدي / بدون شيء]",
  cooldowns: 5,
  dependencies: {
    "fs-extra": "",
    "axios": ""
  }
};

module.exports.run = async function({ api, event, args, Users }) {
  const { senderID, threadID, messageID, type, messageReply, mentions } = event;

  try {
    // 1️⃣ تحديد الهدف
    let uid;
    if (type === "message_reply") uid = messageReply.senderID;
    else if (Object.keys(mentions).length > 0) uid = Object.keys(mentions)[0];
    else if (args[0] && !isNaN(args[0])) uid = args[0];
    else uid = senderID;

    const pathImg = __dirname + `/cache/profile_${uid}.png`;
    const TOKEN = "6628568379%7Cc1e620fa708a1d5696fb991c1bde5662";

    // 2️⃣ جلب اسم الشخص
    const name = await Users.getNameUser(uid);

    const loadingMsg = await api.sendMessage("◈ ───『 جاري جلب الصورة الملكية.. 』─── ◈", threadID, messageID);

    // 3️⃣ جلب الصورة بجودة عالية جداً
    const res = await axios.get(
      `https://graph.facebook.com/${uid}/picture?height=1500&width=1500&access_token=${TOKEN}`,
      { responseType: "arraybuffer" }
    );

    fs.writeFileSync(pathImg, Buffer.from(res.data, "binary"));

    // 4️⃣ تحضير الرسالة النهائية
    const msg = {
      body: `◈ ───『 الـبـروفـايـل الـمـلـكـي 』─── ◈\n\n` +
            `◯ الاسـم : ${name}\n` +
            `🆔 مـعـرف الـحـساب : ${uid}\n` +
            `———————————————\n` +
            `◯ تـم جـلـب الـصورة بـأعـلى جـودة سـيـدي.\n` +
            `———————————————\n` +
            `◈ ─────────────── ◈\n` +
            `│←› بـأوامـر: الـتـوب أيـمـن 👑`,
      attachment: fs.createReadStream(pathImg)
    };

    // 5️⃣ إرسال الرسالة وحذف الصورة بعد الإرسال
    await api.unsendMessage(loadingMsg.messageID);
    return api.sendMessage(msg, threadID, () => {
      if (fs.existsSync(pathImg)) fs.unlinkSync(pathImg);
    }, messageID);

  } catch (e) {
    console.log(e);
    return api.sendMessage(
      "⚠️ سيدي، فشلت في جلب الصورة، قد يكون الحساب مغلقاً أو الرابط معطلاً.",
      threadID,
      messageID
    );
  }
};
