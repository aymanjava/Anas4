module.exports.config = {
  ...
  hasPermssion: 0,
  credits: "Ayman",
  description: "عرض بوسترات مسلسلات عشوائية مع مكافأة من الخزينة",
...
module.exports.run = async ({ api, event, Currencies }) => {
  ...
  const reward = 20; // مكافأة الخدمة (نقاط قليلة كما أمرت سيدي)
  ...
  const cachePath = __dirname + `/cache/series_${senderID}.jpg`;

  const callback = async () => {
    // منح المكافأة المالية
    await Currencies.increaseMoney(senderID, reward);

    let msg = `◈ ───『 الـسـيـنـمـا الـمـلـكـيـة 』─── ◈\n\n` +
              `🎬 إليك اقتراح لمسلسل مميز سيدي.\n` +
              `💰 مكافأة المشاهدة: +${reward}$\n` +
              ` ———————————————\n` +
              `│←› الـراعي: الـتـوب ايـمـن 👑\n` +
              `◈ ──────────────── ◈`;
